...
function ListField(props: { responseState: { setResponse: React.Dispatch<React.SetStateAction<{ [key: string]: any; }>>; response: { [key: string]: any; } }; optionNodes: { listFieldItem: Model.SDCListFieldItem; listFieldItemChildren: JSX.Element[] }[]; children?: any; sdcListField: Model.SDCListField }) {
    ...
    return (
        ...
        >
            ...
            <form onChange={isMultiSelect ? onChangeMultiSelect : onChangeSelect}>
                {props.optionNodes.map((optionnode: { listFieldItem: Model.SDCListFieldItem; listFieldItemChildren: any[] }) => {
                    return <ListFieldItem key={optionnode.listFieldItem.id} responseState={props.responseState} isMultiSelect={isMultiSelect} isSelected={props.responseState.response[props.sdcListField.id] ? props.responseState.response[props.sdcListField.id].indexOf(optionnode.listFieldItem.id) !== -1 : false} sdcListFieldItem={optionnode.listFieldItem}>{optionnode.listFieldItemChildren}</ListFieldItem>
                })}
            </form>
            {props.children}
        </div>
