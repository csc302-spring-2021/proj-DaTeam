...
describe("Verify Form Response Validator", () => {
  ...
  test("Nested List unselected option with children unanswered", (done) => {
    form = Mocks.buildFormNestedList();
    formResponse = Mocks.buildFormResponseDeselectChildren();
    findAnswer(formResponse, "list").responses = ["list-0"];
    expect(validateFormResponse).not.toThrow();
    expect(errors).toHaveLength(0);
    done();
  });
  ...
  test("No answer to text field of an unselected list option", (done) => {
    form = Mocks.buildFormComplete();
    formResponse = Mocks.buildFormResponsePartial2();
    expect(validateFormResponse).not.toThrow();
    expect(errors).toHaveLength(0);
    done();
  });
...
describe("Verify TextField Failures", () => {
  ...
  test("No answer to text field of an selected list option", (done) => {
    form = Mocks.buildFormComplete();
    formResponse = Mocks.buildFormResponsePartial2();
    findAnswer(formResponse, "list-1").responses = ["list-1-t"];
    expect(validateFormResponse).toThrow();
    done();
  });
...
describe("Verify ListField Failures", () => {
  ...
  test("ListField Item w/ selectionDeselectsSiblings enabled and >1 responses Throws Validation Error", (done) => {
    ...
    let qId;
    outer: for (let item of form.children) {
